...
#' Statistical Transformation for Mean Labels
#'
#' This function computes the mean x and y coordinates for a group and displays
#' a label at the computed mean position.
#'
...
#' the default mapping at the top level of the plot. You must supply \code{mapping}
...
#' \code{ggplot()}. A \code{data.frame}, or other object, will override the plot data.
#' A \code{function} will be called with a single argument, the plot data
#' @param geom The geometric object to use to display the data. Defaults to "label"
...
#' @param digits Number of digits used when formatting the default mean-coordinate
#' label. Ignored when a \code{label} aesthetic is supplied
#' @param show.legend Logical. Should this layer be included in the legends?
#' \code{NA}, the default, includes if any aesthetics are mapped. \code{FALSE}
#' never includes, and \code{TRUE} always includes
#' @param inherit.aes If \code{FALSE}, overrides the default aesthetics, rather than
#' combining with them
...
#' or \code{size = 3}
#' @return A ggplot2 layer that adds labels to mean positions on a plot
#' @examples
#' library(ggplot2)
#' data <- data.frame(
#'   x = rnorm(100),
...
#' )
#'
#' ggplot(data, aes(x, y, colour = group, label = group)) +
#'   geom_point() +
#'   stat_mean_label()
#'
#' ggplot(data, aes(x, y, colour = group, group = group)) +
#'   geom_point() +
#'   stat_mean_label(digits = 2)
#' @export
stat_mean_label <- function(
  mapping = NULL,
  data = NULL,
  geom = "label",
  position = "identity",
...
#' @import ggplot2
StatMeanLabel <- ggproto(
  "StatMeanLabel",
  Stat,
  compute_group = function(data, scales, na.rm = FALSE, digits = 2) {
    x <- mean(data$x, na.rm = na.rm)
    y <- mean(data$y, na.rm = na.rm)
